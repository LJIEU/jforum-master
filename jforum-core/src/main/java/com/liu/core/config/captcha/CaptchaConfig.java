...
public class CaptchaConfig {
    ...
    // 本地图片
    private final static String IMG_PATH = "classpath:/captcha/%s.jpg";
    ...
    public static BufferedImage getBufferedImage(Integer place) {
        try {
            ...
            //获取网络资源图片
            if (0 == place && StrUtil.isNotEmpty(IMG_URL)) {
            ...
            //获取本地图片
                ...
                nonce = getNonceByRange(0, imageNum());
                String imgPath = String.format(IMG_PATH, nonce);
                File file = new File(imgPath);
                return ImageIO.read(file);
            }
        } catch (Exception e) {
            throw new ServiceException("获取拼图资源失败");
            //异常处理
        }
    }
    ...
    private static int[][] getBlockData(int blockWidth, int blockHeight, int blockRadius) {
        ...
        //凸/凹1
        int face1 = RandomUtil.getRandom().nextInt(0, 4);
        //凸/凹2
        int face2;
        //保证两个凸/凹不在同一位置
        do {
            face2 = RandomUtil.getRandom().nextInt(0, 4);
        } while (face1 == face2);
        //获取凸/凹起位置坐标
    ...
    private static int imageNum() {
        String dirPath = CaptchaConfig.IMG_PATH.substring(0, CaptchaConfig.IMG_PATH.length() - "%s.jpg".length());
        Path dir = Paths.get(dirPath);
        // 获取指定目录下的所有文件列表 不包括子目录和目录本身
        try {
            return (int) Files.list(dir)
                    .filter(path -> !Files.isDirectory(path) && path.toString().endsWith(".jpg")).count();
        } catch (IOException e) {
            throw new ServiceException("获取本地图片列表长度失败!");
        }
    }
